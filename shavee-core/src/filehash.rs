...
pub fn get_filehash(
    ...
    if file.starts_with("https://") || file.starts_with("http://") || file.starts_with("sftp://") {
        ...
        get_filehash_http_sftp(file, port, size, salt).map_err(Into::into)
    } else {
        crate::trace("File location is local.");
        get_filehash_local(file, size, salt).map_err(Into::into)
    }
}

fn get_filehash_local(file: &str, size: Option<u64>, salt: &[u8]) -> Result<Vec<u8>, io::Error> {
    ...
    let finalhash = crate::password::hash_argon2(&hashbuffer[..], salt).expect("File Hash Error");
...
fn get_filehash_http_sftp(
    ...
    let finalhash =
        crate::password::hash_argon2(&hashbuffer[..], salt).expect("Online File Hash Error");
    Ok(finalhash)
}
